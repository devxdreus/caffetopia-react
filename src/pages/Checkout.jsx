  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      const newData = data.map((entry) => {
        const table =
          localStorage.getItem(`order-${entry.id}-table`) || entry.table;
                  ...
                  >
                    ...
                    {availableNumber.map((number) => {
                      return number.available ? (
                        <option key={number.table} value={number.table}>
                          Table {number.table}
                        </option>
                      ) : (
                        <option disabled key={number.table}>
                          Table {number.table} (booked)
                        </option>
                      );
                    })}
